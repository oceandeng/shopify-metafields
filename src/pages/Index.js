...
class App extends React.Component {
    ...
    handleGetShopData() {
        window.__PERMISSION__ = {
            ...
            hasImport: true,
            hasBulkExport: true,
            editorFieldsOptions: [{enable: true}],
        };
        // const getShop = () => { return window.axios.get(`${window.appEnvironment.apiURL}shopify/shop`) }
        // const getCurrentPlan = () => { return window.axios.get(`${window.appEnvironment.apiURL}plan/current`) }
        // const getPlan = () => { return window.axios.get(`${window.appEnvironment.apiURL}plan`) }
        // window.axios.all([getShop(), getCurrentPlan(), getPlan()]).then(window.axios.spread((shop, current, plan) => {
